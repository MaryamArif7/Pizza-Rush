...
export default function Recovery() {
  ...
  async function resendOTP() {
    try {
      const sentOTP = await generateOTP(username);
      toast.success("OTP has been sent to your email!");
      console.log(sentOTP);
    } catch (error) {
      ...
      <div className="flex justify-center items-center h-screen">
        <div className="bg-[rgba(255,255,255,0.55)] rounded-[16px] shadow-[0_4px_30px_rgba(71,71,71,0.11)] backdrop-blur-[7.1px]  border-[rgba(255,255,255,0.3)] border-4 border-gray-50 shrink-0 h-[75%] w-[30%]  py-20 px-7 min-w-max">
          <div className="title flex flex-col items-center">
            <h4 className="bg-gradient-to-r from-yellow-500 to-red-600 bg-clip-text text-transparent text-4xl font-bold">
              Recovery
            </h4>
          ...
          <form className="pt-10" onSubmit={onSubmit}>
            <div className="textbox flex flex-col items-center gap-3">
              ...
              <button
                className="border bg-gradient-to-r from-yellow-400 to-red-600 text-lg  hover:bg-red-700 mt-2 text-center rounded-lg  px-10 py-3 shadow-sm "
                type="submit"
              >
                Recover
              </button>
          ...
          <div className="text-center py-4">
            <span>
              Cant get OTP?{" "}
              <button onClick={resendOTP} className="text-red-600">
                Resend
              </button>
            </span>
          </div>
